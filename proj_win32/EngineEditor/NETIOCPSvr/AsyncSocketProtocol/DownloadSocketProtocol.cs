...
namespace AsyncSocketServer
{
    public class DownloadSocketProtocol : BaseSocketProtocol
    {
        ...
        public bool DoDir()
        {
            ...
            {
                if (parentDir == "")
                    parentDir = fileDirection;
                else
                    parentDir = Path.Combine(fileDirection, parentDir);
        ...
        public bool DoFileList()
        {
            ...
            {
                if (dirName == "")
                    dirName = fileDirection;
                else
                    dirName = Path.Combine(fileDirection, dirName);
        ...
        public bool DoDownload()
        {
                ...
                & m_incomingDataParser.GetValue(ProtocolKey.FileSize, ref fileSize) & m_incomingDataParser.GetValue(ProtocolKey.PacketSize, ref packetSize))
            {
                ...
                Debug.WriteLog.Log(eLogLevel.ell_Info,"Start download file: " + fileName);
                ...
                if (File.Exists(fileName))
                {
                    ...
                    else
                    {
                        m_outgoingDataAssembler.AddFailure(ProtocolCode.FileIsInUse, "");
                         Debug.WriteLog.Log(eLogLevel.ell_Error,"Start download file error, file is in use: " + fileName);
                    }
                }
                else
                {
                ...
                lock (m_asyncSocketServer.DownloadSocketProtocolMgr)
                {
                    ...
                    for (int i = 0; i < m_asyncSocketServer.DownloadSocketProtocolMgr.Count(); i++)
                    {
                        ...
                        if (fileName.Equals(downloadSocketProtocol.FileName, StringComparison.CurrentCultureIgnoreCase))
                        {
                            ...
                            {
                                m_asyncSocketServer.ServerLogic.CloseClientSocket(downloadSocketProtocol.AsyncSocketUserToken);
                            }
                            result = false;
                        }
                    }
        ...
        public override bool SendCallback()
        {
            ...
            if (m_fileStream != null)
            {
                ...
                else
                {
                    ...
                    {
                        Debug.WriteLog.Log(eLogLevel.ell_Info, "End download file: " + m_fileName);
                        m_fileStream.Close();
                        m_fileStream = null;
                        m_fileName = "";
                        m_sendFile = false;
